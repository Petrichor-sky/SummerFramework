...
public class PropertyResolver {
    ...
    public PropertyResolver(Properties props) {
        this.properties.putAll(System.getenv()); //存入环境变量
        Set<String> names = props.stringPropertyNames(); //存入properties
        for (String name : names) {
            this.properties.put(name, props.getProperty(name));
        }
        //以上存储了所有的配置项，包括环境变量和properties文件中的属性
    ...
    public String getProperty(String key) {
        ...
        //keyExpr 不为null 说明是 ${key} 或者 ${key:defaultValue} 这种形式的配置项
        //为null说明是 getProperty("app.title")
        if (keyExpr != null) {
    ...
    PropertyExpr parsePropertyExpr(String key) {
        if (key.startsWith("${") && key.endsWith("}")) {
            // 是否存在defaultValue?  ${abc.xyz:defaultValue}
            int n = key.indexOf(':');
            if (n == (-1)) {
